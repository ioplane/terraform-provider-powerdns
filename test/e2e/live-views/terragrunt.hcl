...
terraform {
  ...
  source = "git::http://e2e:${get_env("E2E_TOKEN")}@127.0.0.1:19300/e2e/dns-modules.git//modules/views?ref=main"
}
...
remote_state {
  ...
  config = {
    bucket = "e2e-state"
    key    = "dns-views/terraform.tfstate"
    region = "us-east-1"
...
generate "provider" {
  ...
  if_exists = "overwrite_terragrunt"
  contents  = <<-EOT
    # The LMDB instance, because views and networks are unimplemented by gpgsql.
    provider "powerdns" {
      server_url = "http://127.0.0.1:18091"
      api_key    = "labapikey"
    }
  EOT
}
