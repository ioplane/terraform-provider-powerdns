...
#
# Pinned to Terraform: actions are a 1.14 feature and OpenTofu does not have
# them. Every other unit runs on whatever Terragrunt selects, which is tofu.
...
terraform {
  ...
  source = "git::http://e2e:${get_env("E2E_TOKEN")}@127.0.0.1:19300/e2e/dns-modules.git//modules/imperative?ref=main"
}
...
remote_state {
  ...
  config = {
    bucket = "e2e-state"
    key    = "dns-imperative/terraform.tfstate"
    region = "us-east-1"
...
inputs = {
  ...
  autoprimary_ip = "192.0.2.200"
  autoprimary_ns = "primary.e2e.example."
