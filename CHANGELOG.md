...
- The module remote serves TLS, with a self-signed certificate generated by
  `task e2e:up` and trusted for that one host — a module source is fetched over
  HTTPS everywhere it matters, and a fixture that authenticates over plain HTTP
  is a shape people copy. A scenario asserts the certificate is verified rather
  than that verification is switched off.
